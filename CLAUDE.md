...
- **Unit of Work (ADR-027)**: Command handlers MUST call `uow.CommitAsync(ct)` — never `dbContext.SaveChangesAsync()` directly. `IUnitOfWork` is defined in `Base.Infrastructure`. Domain events split into two types: **pre-commit** (`IPreCommitDomainEvent` — dispatched INSIDE the open DB transaction before `SaveChanges`, for atomic side effects) and **post-commit** (plain `IDomainEvent` — dispatched AFTER transaction commit, failures logged only). `UnitOfWork` implementation in `MarketNest.Web.Infrastructure/Persistence/`. See `docs/backend-patterns.md` §22 and ADR-027.
