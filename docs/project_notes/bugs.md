...
### 2026-05-01 - Background job crash: InvalidOperationException on UnitOfWork disposal
- **Issue**: `notifications.cleanup-expired` background job failed with `InvalidOperationException: 'MarketNest.Web.Infrastructure.UnitOfWork' type only implements IAsyncDisposable. Use DisposeAsync to dispose the container.`
- **Root Cause**: `CleanupExpiredNotificationsJob` manually created a scoped DI container with `using var scope = serviceProvider.CreateScope()` (synchronous), but then injected `IUnitOfWork` which only implements `IAsyncDisposable`. When the `using` block exited, it tried to call `.Dispose()` synchronously on a scope containing async-only disposables, causing an exception.
